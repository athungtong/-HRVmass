function varargout = RobustRdetection(varargin)
% ROBUSTRDETECTION MATLAB code for RobustRdetection.fig
%      ROBUSTRDETECTION, by itself, creates a new ROBUSTRDETECTION or raises the existing
%      singleton*.
%
%      H = ROBUSTRDETECTION returns the handle to a new ROBUSTRDETECTION or the handle to
%      the existing singleton*.
%
%      ROBUSTRDETECTION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ROBUSTRDETECTION.M with the given input arguments.
%
%      ROBUSTRDETECTION('Property','Value',...) creates a new ROBUSTRDETECTION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before RobustRdetection_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to RobustRdetection_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help RobustRdetection

% Last Modified by GUIDE v2.5 10-Oct-2014 15:31:36

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @RobustRdetection_OpeningFcn, ...
                   'gui_OutputFcn',  @RobustRdetection_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before RobustRdetection is made visible.
function RobustRdetection_OpeningFcn(hObject, eventdata, handles, varargin)
...
handles.fid=-1;
handles.ProjectName='';
set(handles.figure1,'Name','untitled - Robust R wave detection');
...
function varargout = RobustRdetection_OutputFcn(hObject, eventdata, handles) 
...
handles.inputfullfile=contents{get(hObject,'Value')};
[filepath, filename]=fileparts(handles.inputfullfile);
...
if  isempty(handles.Results{selfile}.RRinfo.R_time)
    handles=setforplot(handles,0);
%     warndlg('No R time information available. Please click Detect R in all ECG files.');    
%     return;
    handles=start(handles,selfile);
end
...
%     handles.clickRR=0;
    ...
    if ~isempty(tick)
        xlim([tick(1) tend/60+handles.epoch_length/2]); 
    end
    grid on;
...
function handles=prepareECGfile(handles)
    ...
    handles.ECGfullfile=handles.Results{handles.selectedfilenum}.rawfilename;
    if handles.fid~=-1
        fclose(handles.fid);
        % close current file and process to hrvfilemenu the new one
    end
    handles.fid = fopen(handles.ECGfullfile);
    handles.currentECGfile=handles.ECGfullfile;
    ...
    if handles.fid==-1 
        ...
        handles.ECGfullfile=fullfile(filepath,filename);
        handles.fid = fopen(handles.ECGfullfile);
        handles.currentECGfile=handles.ECGfullfile;
    end
...
%     handles.param=handles.Results{selfile}.param;
    handles.ECGch=findchannelnum(handles.ECGfullfile,handles.set.ecgch.chnum);
...
fprintf(handles.logfid,'%s\r',['Process start: ' datestr(now)]);
    ...
    drawnow;
    handles.inputfullfile=handles.fname{i};
    fid=fopen(handles.inputfullfile);
    if fid==-1
        text=['Cannot load ' handles.inputfullfile];
        warning(text);
    ...
    if fid==-1
        ...
        handles.Results{i}.rawfilename=handles.inputfullfile;

    else
        handles.Results{i}.rawfilename=handles.inputfullfile;
        handles.inputfullfile=handles.foutname;
%         handles=loadRfile(handles);
        handles.R_time = load(handles.inputfullfile);
...
handles.inputfullfile=handles.fname{handles.selectedfilenum};
...
handles.inputfullfile=char(handles.fname(1));
...
if (Loc(3)>yLim(1) & Loc(3)<yLim(2) & Loc(1)>xLim(1) & Loc(1)<xLim(2))
    ...
    if strcmp(get(gcbf, 'SelectionType'),handles.set.click) 
        ...
        % Semi Auto, will work if isdetrend and issquare
            if handles.isdetrend 
...
handles.inputfullfile=char(handles.fname(handles.selectedfilenum));
[filepath, filename]=fileparts(handles.inputfullfile);
handles.foutname=[fullfile(filepath,filename) '_Rtime.txt'];
set(handles.OutputFiletxt,'string',handles.foutname);
handles=updateplot(handles);
